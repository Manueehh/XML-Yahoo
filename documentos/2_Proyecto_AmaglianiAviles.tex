...
\vspace{1cm}
\begin{lstlisting}[language=XMLL,caption={Estructura de un documento del corpus.},label=lst:xml, xleftmargin=0.5cm]
...
%% ============================================
En la siguiente sección se presentan las tecnologías utilizadas para el desarrollo del proyecto,
con una breve descripción y la estructura del repositorio que hemos desarrollado para organizar el código y los datos.
...
\begin{lstlisting}[caption={Estructura del repositorio.}, xleftmargin=0.5cm]
YahooAnswers-XML/
|-- src/
...
\begin{itemize}
  \item extraer\_categorias.py: script para extraer preguntas por categor\'ia.
  \item extraer\_preguntas.py: script para extraer preguntas individuales.
  \item cargar\_existdb.py: script para cargar XML en eXist-db.
  \item ejecutar\_xquery.py: script para ejecutar consultas XQuery.
\end{itemize}

Las carpetas principales son:
- \textbf{xquery/}: carpeta con las cuatro consultas XQuery desarrolladas.
- \textbf{xsd/}: carpeta con el esquema XSD original y ampliado.
- \textbf{xslt/}: carpeta con las transformaciones XSLT a HTML.
- \textbf{data/}: carpeta con los cuatro ficheros XML extra\'idos por categor\'ia.
...
\begin{lstlisting}[language=XMLL,caption={Fragmento del XSD original.},label=lst:xsd, xleftmargin=0.5cm]
...
\begin{lstlisting}[language=XMLL,caption={Plantilla XSLT por documento.},label=lst:xslt, xleftmargin=0.5cm]
...
\begin{lstlisting}[caption={Colecciones en eXist-db.}, xleftmargin=0.5cm]
/db/yahooanswers/
...
\begin{lstlisting}[language=XQuery,caption={Estadisticas por categoria.}, xleftmargin=0.5cm]
let $es := collection(
...
\begin{lstlisting}[language=XQuery,caption={Calculo de simetria.}, xleftmargin=0.5cm]
let $todos := ($es, $it)
...
\begin{lstlisting}[language=XQuery,caption={Analisis de completitud.}, xleftmargin=0.5cm]
let $todos := ($es,$it)
return <completitud>{
